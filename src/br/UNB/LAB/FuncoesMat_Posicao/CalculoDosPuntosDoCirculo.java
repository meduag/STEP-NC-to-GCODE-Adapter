package br.UNB.LAB.FuncoesMat_Posicao;
...
public class CalculoDosPuntosDoCirculo {
	...
	public static ArrayList<Double> DiamSeg = new ArrayList<Double>();
	public static ArrayList<Double> ProfSegm = new ArrayList<Double>();
	public static ArrayList<Double> ProfOff = new ArrayList<Double>();
	...
	public static void CalcularPuntosdaSemiCircunferencia(double diametroFuro_DIni, double diametroTool_Prof){
		double PontoX=0, PontoY=0; 
		DecimalFormat df = new DecimalFormat("0.0000");
		//double radioFuro=1/2;
		int valPasAng = (int)Gcode.AvanceDoAguloSemiCircunferencia;
		cAng=0;
		//System.out.println();
		do{
			x=PontoX+(diametroFuro_DIni)*Math.cos(Math.toRadians(cAng));
			x = Double.parseDouble(df.format(x).replaceAll(",", "."));//verificar si da error
			DiamSeg.add(x);
			
			y=PontoY+(diametroTool_Prof/2)*Math.sin(Math.toRadians(cAng));
			y = (Math.round(y*10000)/10000.0);
			ProfSegm.add(y);
			//System.out.println("Ang: "+cAng +"   D"+df.format(x)+"    Z"+df.format(y));
			//System.out.println("Ang: "+cAng +"   D"+DiaSeg[cAng/graAvan]+"    Z"+ProfSeg[cAng/graAvan]);
		...
		public static void CalcProfOffSemiCir(double diametroFuro_DIni, double diametroTool_Prof){
		double PontoX=0, PontoY=0; 
		DecimalFormat df = new DecimalFormat("0.0000");
		//double radioFuro=1/2;
		int valPasAng = (int)Gcode.AvanceDoAguloSemiCircunferencia;
		...
		do{
			...
			y=PontoY+(diametroTool_Prof)*Math.sin(Math.toRadians(cAng));
			y = (Math.round(y*10000)/10000.0);
			ProfOff.add(y);
			//System.out.println("Ang: "+cAng +"   D"+df.format(x)+"    Z"+df.format(y));
			//System.out.println("Ang: "+cAng +"   D"+DiaSeg[cAng/graAvan]+"    Z"+ProfSeg[cAng/graAvan]);
